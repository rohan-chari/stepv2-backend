...
function valid(payload, kind, proof, now, horizon) {
  ...
  for (const row of payload.events) {
    ...
    seen.add(identity);
    const orderKey = JSON.stringify([row.startsAt, row.id, row.entitlementId, row.impactId, row.userId]);
...
function materialize(global, local, proof, now, horizon) {
  ...
  // Retain its full total order (including per-user entitlement/impact keys),
