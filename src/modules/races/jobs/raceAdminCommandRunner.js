  ...
  return async function runRaceAdminCommand() {
    ...
    try {
      const execution = await prisma.$transaction(async (tx) => {
        ...
        return { outcome, raceId: command.race_id };
      }, { timeout: 30_000, maxWait: 10_000 });
      if (execution.outcome?.mutated) {
        await Promise.all([
          invalidateRaceProgress(execution.raceId).catch(() => false),
          invalidateTournament(execution.outcome.tournamentId).catch(() => false),
          ...execution.outcome.participantUserIds.map((userId) =>
            invalidateRaceListUser(userId).catch(() => false)
          ),
        ]);
      }
      return true;
