...
async function reconcileIdentity({db,config,provider,identity,goldTransactionId=null}){
 ...
 try{const history=await provider.getCustomerHistory(identity,{goldTransactionId});const result=await applyHistory({db,config,identity,token,history});await releaseLease(db,identity.id,token,result.verificationDeferred?{failed:true,message:'BILLING_SOURCE_VERIFICATION_PENDING'}:{});return result;}
