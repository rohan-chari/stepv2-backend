const PROJECTION_VERSION = "races-tab-open-projection-v2";

const REQUIRED_COVERAGE_VARIANTS = Object.freeze([
...
function placementFor(row) {
  return {
    value: integer(row?.myPlacement),
    hidden: boolean(row?.myPlacementHidden),
...
function teamFor(row) {
  ...
  return {
    size: integer(row.teamSize),
    callerTeam: nullableString(row.myTeam),
    teamA: {
      name: nullableString(row.teams?.teamA?.name ?? row.teamAName),
      memberCount: integer(row.teams?.teamA?.memberCount),
      totalSteps: integer(row.teams?.teamA?.totalSteps ?? row.teamATotalSteps),
    },
    teamB: {
      name: nullableString(row.teams?.teamB?.name ?? row.teamBName),
      memberCount: integer(row.teams?.teamB?.memberCount),
      totalSteps: integer(row.teams?.teamB?.totalSteps ?? row.teamBTotalSteps),
    },
    totalsAsOf: timeValue(row.teams?.asOf),
    winnerTeam: nullableString(row.winnerTeam),
  };
}

function ordinaryRow(row, bucket, index) {
  return {
    rowKey: `ordinary.${bucket}.${index}`,
    name: nullableString(row?.name),
    status: nullableString(row?.status),
    creatorDisplayValue: nullableString(row?.creator?.displayName),
    isCreator: boolean(row?.isCreator),
    participantCount: integer(row?.participantCount),
    myStatus: nullableString(row?.myStatus),
    maxDurationDays: integer(row?.maxDurationDays),
    kind: row?.isTeamRace === true ? "team" : "classic",
    isFavorite: boolean(row?.isFavorite),
    favoriteOrder: timeValue(row?.favoritedAt),
    ...
    time: {
      ...
      scheduledStartAt: timeValue(row?.scheduledStartAt),
...
function inventoryFor(row, rowKey) {
  ...
  return {
    ...
    mysteryBoxCount: Number.isInteger(row?.mysteryBoxCount)
      ? row.mysteryBoxCount : slots.filter((item) => item?.status === "MYSTERY_BOX").length,
...
function effectsFor(row, rowKey) {
  ...
  for (const effect of array(row?.myActiveEffects)) {
    // The compact GET /races contract contains only active effects and omits
    // the durable status field. Captured fixtures may still carry ACTIVE.
    if (typeof effect?.type !== "string" || effect.status != null && effect.status !== "ACTIVE") continue;
    const bucket = NEGATIVE_EFFECTS.has(effect.type) ? result.negative : result.positive;
...
function tournamentRow(row, bucket, index, viewerUserId) {
  return {
    rowKey: `tournaments.${bucket}.${index}`,
    name: nullableString(row?.name),
    callerStatus: nullableString(row?.myStatus),
    status: nullableString(row?.status),
    renderState: tournamentRenderState(row, viewerUserId),
    hasCurrentMatchRaceId: typeof row?.myCurrentMatchRaceId === "string",
    isFavorite: boolean(row?.isFavorite),
    favoriteOrder: timeValue(row?.favoritedAt),
    bracketSize: integer(row?.bracketSize),
    acceptedCount: integer(row?.acceptedCount),
    currentRound: integer(row?.currentRound),
    totalRounds: integer(row?.totalRounds),
    prize: {
      championPrizeCoins: integer(row?.championPrizeCoins),
      potCoins: integer(row?.potCoins),
      funded: row?.prizePool?.funded === true,
      coins: integer(row?.prizePool?.coins),
    },
    callerIdentity: {
      displayName: nullableString(row?.myIdentity?.displayName),
...
function observedCoverageVariants(projection = {}) {
  const seen = new Set();
  ...
  const stateVariant = {
    invite: "tournament_invite", lobby: "tournament_lobby",
  ...
  for (const rows of Object.values(projection.tournaments || {})) {
  ...
  for (const row of array(projection.tournamentMatchByTournament)) {
    ...
    if (array(row.inventory?.heldTypedItems).length) {
      seen.add("tournament_match_inventory_held_typed");
    }
    if (Number(row.inventory?.mysteryBoxCount) > 0) {
      seen.add("tournament_match_inventory_mystery_box");
    }
    if (Number(row.inventory?.queuedBoxCount) > 0) {
      seen.add("tournament_match_inventory_queued_box");
    }
  }
...
function tournamentMatch(row, rowKey) {
  ...
  return {
    ...
    endsAt: timeValue(match.endsAt),
    placement: placementFor(match),
    inventory: inventoryFor(match, rowKey),
  };
}

function projectRacesTabPayload({ core = {}, discovery = {}, friends = null,
  ...
  return {
    ...
    ordinary,
    ordinaryInventoryByRace,
    ordinaryEffectsByRace,
    tournaments,
    tournamentMatchByTournament,
    discovery: { publicRaceCount: integer(discovery?.publicRaceCount) },
    friends: {
      shouldRequest: friendsShouldRequest === true,
      expectedCount: friendsShouldRequest ? array(friends?.friends).length : 0,
      expectedContract: friendsShouldRequest
        ? nullableString(friends?.contract) : "friends-summary-v1",
    },
  };
}
...
function compareRacesTabProjection(expected, observed, { sampleLimit = 50 } = {}) {
  ...
  const visit = (left, right, path = "") => {
    if (Array.isArray(left)) {
      if (!Array.isArray(right)) return mismatch(path, left, right);
