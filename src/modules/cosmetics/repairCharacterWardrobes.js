...
async function repairCharacterWardrobes({
  ...
  for (const { id: userId } of users) {
    const result = await withWriter(
      ...
      async (tx, state) => {
        const key = activeKey(state.equipment),
          saved = state.wardrobes.find((w) => w.characterKey === key);
        if (!saved || sameSlots(slotMap(saved.items), slotMap(state.equipment)))
          return { appearanceChanged: false, mismatch: false };
        if (apply) {
          await writeWardrobe(tx, userId, key, saved, state.equipment);
    ...
    if (result.appearanceChanged) repaired++;
  }
  return {
    usersChecked: users.length,
    mismatches,
    repaired,
    nextCheckpoint: users.length === limit ? users.at(-1).id : null,
  };
}
