...
function renderReport(summary) {
  ...
  const racesBackground = isRaces ? summary.levels.map((row) =>
    `| ${row.rate}/sec | ${row.racesTabOpen?.discovery?.started ?? "—"}/${row.racesTabOpen?.discovery?.completed ?? "—"}/${row.racesTabOpen?.discovery?.errors ?? "—"} | ${row.racesTabOpen?.discovery?.latencyMs?.p95 ?? "—"}/${row.racesTabOpen?.discovery?.latencyMs?.p99 ?? "—"} | ${row.racesTabOpen?.friends?.started ?? "—"}/${row.racesTabOpen?.friends?.completed ?? "—"}/${row.racesTabOpen?.friends?.errors ?? "—"} | ${row.racesTabOpen?.friends?.latencyMs?.p95 ?? "—"}/${row.racesTabOpen?.friends?.latencyMs?.p99 ?? "—"} |`).join("\n") : "";
