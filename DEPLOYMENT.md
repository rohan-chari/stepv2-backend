...
RACE_PAYOUT_DOUBLE_MAX_BONUS_COINS=100
...
CLI allowlist. Their exact npm lifecycle name and exact committed
script path select the `maintenance` pool, which defaults to 2 connections and
may be set to a canonical 1–5 with `DATABASE_POOL_MAX_MAINTENANCE`. Direct
