-- Active work still accepts completed rolling-worker inputs so the v2 worker
-- can fail the whole group closed. Pending membership is accepted only before
-- capture; every terminal work state rejects incompatible late mutations.
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    IF work_status = 'WAITING_SYNC'
       OR (work_status IN ('QUEUED', 'PROCESSING', 'WAITING_RACES')
  ...
  ELSE
    ...
    IF work_status IN ('QUEUED', 'PROCESSING', 'WAITING_RACES')
       ...
       AND NEW.attribution_version = OLD.attribution_version
       AND NEW.status IN ('FINAL', 'UNSCORABLE', 'EXPIRED_UNDELIVERED') THEN
      RETURN NEW;
    END IF;
