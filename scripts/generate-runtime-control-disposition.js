...
const envMetadata = {
  ...
  QUEUE_REDIS_URL: ["deployment_config", null, "dedicated Redis connection URL for durable queue transport and stream trimming; required by queue-owning processes", "queue_transport", null],
...
function environmentKind(name, metadata) {
  if (
    /(?:_CONCURRENCY|_MS|_HOURS|_DAYS|_COINS|_CAP|_RATE|_PERCENT|_THRESHOLD|_MAX_OPENS|_EXPECTED_OWNERS|_EXPIRES_SECONDS|_MULT_(?:SHORT|MID|LONG)|_VERSION|^PORT|^DATABASE_POOL_(?:MAX_(?:HTTP|STEP|RESOLUTION|EVENT|NOTIFICATION|CRON|ALL|DEFAULT|MAINTENANCE)|TOTAL_BUDGET))$/.test(name)
  ) {
