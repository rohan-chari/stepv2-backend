...
describe("Ghost Pepper cannot be stacked", () => {
  ...
  it("enforces the one-hour post-effect cooldown, then allows a fresh pepper at the boundary", async () => {
    ...
    const duringCooldown = await usePowerup(alice.token, raceId, second.id);
    assert.equal(duringCooldown.status, 409);
    assert.equal((await duringCooldown.json()).code, "POWERUP_COOLDOWN");
    assert.equal(
      (await prisma.racePowerup.findUnique({ where: { id: second.id } })).status,
      "HELD",
      "cooldown rejection must not consume the held pepper"
    );

    await prisma.powerupUsageState.update({
      where: {
        raceId_userId_powerupType: {
          raceId,
          userId: alice.userId,
          powerupType: "GHOST_PEPPER",
        },
      },
      data: { nextUsableAt: new Date(Date.now() - 1) },
    });

    const atBoundary = await usePowerup(alice.token, raceId, second.id);
...
it("treats a time-expired ACTIVE row as ended, but still enforces the post-effect cooldown", async () => {
    ...
    const response = await usePowerup(alice.token, raceId, second.id);
    assert.equal(response.status, 409);
    assert.equal(
      (await response.json()).code,
      "POWERUP_COOLDOWN",
      "the stale ACTIVE effect no longer blocks as an active pepper; the remaining block is the usage cooldown"
    );
    assert.equal(
      (await prisma.racePowerup.findUnique({ where: { id: second.id } })).status,
      "HELD"
