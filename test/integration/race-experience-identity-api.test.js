...
describe("race experience + discoverable identity — locked HTTP contract", () => {
  ...
  beforeEach(async () => {
    ...
    await setFlag("quickRaceShareAutoFriendEnabled", false);
    await setFlag("redisCacheAuthMeEnabled", false);
    // These three graduated behaviors are permanent. Keep the historical DB
    // rows above as stale-row fixtures, while the test-only in-memory seam
    // models the production constants for the public HTTP assertions below.
