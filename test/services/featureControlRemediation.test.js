...
const { PERMANENT_FLAGS } = require("../../src/shared/config/appSettings");
const {
...
test("retired AppSetting cleanup enforces the seven-day rollback window", () => {
  const now = new Date("2026-08-20T12:00:00Z");
  assert.throws(
    () => assertCleanupDelayElapsed("2026-08-14T12:00:01Z", now),
    /seven-day rollback window/,
  );
  assert.doesNotThrow(() =>
    assertCleanupDelayElapsed("2026-08-13T12:00:00Z", now),
  );
});

test("retired AppSetting apply requires deploy, no-old-worker, and restore-drill evidence", async () => {
  const now = new Date("2026-08-20T12:00:00Z");
  const prisma = {
    appSetting: {
  ...
  await assert.rejects(
    cleanupRetiredAppSettings({
      prisma,
      apply: true,
      evidenceTimestamp: "2026-08-13T12:00:00Z",
      now,
    }),
    /deployment and restore-drill evidence/,
  );
  assert.throws(
    () => assertCleanupEvidence({
      deploymentCompletedAt: "2026-08-12T10:00:00Z",
      noOldWorkerObservedAt: "2026-08-13T12:00:00Z",
    }, now),
    /restore drill/,
  );
  const evidence = {
    deploymentCompletedAt: "2026-08-12T10:00:00Z",
    noOldWorkerObservedAt: "2026-08-13T12:00:00Z",
    restoreDrillCompletedAt: "2026-08-13T10:00:00Z",
    restoreDrillId: "restore-drill-2026-08-13",
  };
  assert.doesNotThrow(() => assertCleanupEvidence(evidence, now));
  assert.deepEqual(
    await cleanupRetiredAppSettings({ prisma, apply: true, evidence, now }),
    { keys: Object.keys(PERMANENT_FLAGS).sort(), rows: [], deleted: 0 },
  );
});
