...
const rows = [
  ...
  { powerupType: "HITCHHIKE", name: "Hitchhike", description: "Copy half of the target's eligible steps.", shortDescription: "Half of eligible steps copied", upgradeTierLabels: [] },
...
test("catalog copy and Quicksand visibility are request-capability scoped", async () => {
  ...
  assert.match(legacy.powerups.find((p) => p.type === "HITCHHIKE").description, /eligible step/);
  ...
  assert.match(capable.powerups.find((p) => p.type === "HITCHHIKE").description, /half.*eligible.*boosts and reversals/i);
